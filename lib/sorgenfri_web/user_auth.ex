defmodule SorgenfriWeb.UserAuth do
  use SorgenfriWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_sorgenfri_web_user_remember_me"
  ...
  def log_in_user(conn, user, params \\ %{}) do
    token = Accounts.generate_account_session_token(user)
  ...
  def log_out_user(conn) do
    ...
    user_token && Accounts.delete_account_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      SorgenfriWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    user = user_token && Accounts.get_account_by_session_token(user_token)
  ...
  the current_user:
      ...
      defmodule SorgenfriWeb.PageLive do
        use SorgenfriWeb, :live_view

        on_mount {SorgenfriWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SorgenfriWeb.UserAuth, :ensure_authenticated}] do
  ...
  defp mount_current_user(socket, session) do
    Phoenix.Component.assign_new(socket, :current_user, fn ->
      if user_token = session["user_token"] do
        Accounts.get_account_by_session_token(user_token)
      end
    end)
  end
