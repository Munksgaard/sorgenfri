defmodule SorgenfriWeb.UserSessionControllerTest do
  ...
  describe "POST /accounts/log_in" do
    test "logs the user in", %{conn: conn, user: user} do
      conn =
        post(conn, ~p"/accounts/log_in", %{
          "account" => %{"email" => user.account.email, "password" => valid_user_password()}
        })
      ...
      assert response =~ user.account.email
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_sorgenfri_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "DELETE /accounts/log_out" do
    test "logs the user out", %{conn: conn, user: user} do
      conn = conn |> log_in_user(user) |> delete(~p"/accounts/log_out")
      assert redirected_to(conn) == ~p"/"
